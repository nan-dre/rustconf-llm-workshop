...
mod tools {
        ...
        vec![
            ...
            Tool {
                ...
                function: Function {
                    ...
                    description: "Searches Wikipedia for information.".to_string(),
                    parameters: json!({
                        "type": "object",
                        "properties": {
                            "query": { "type": "string", "description": "The search query." }
    ...
    async fn get_page_intro(title: &str) -> Result<String, Box<dyn Error>> {
        let client = reqwest::Client::new();
        ...
        // Extract the first page's extract
    ...
    async fn search_wikipedia(query: &str) -> Result<String, Box<dyn Error>> {
        let client = reqwest::Client::new();
        let url = "https://en.wikipedia.org/w/api.php";
        let params = [
            ("action", "query"),
        ...
        let title = response["query"]["search"]
            .as_array()
        ...
        info!("> Wikipedia search result: {}", title);

        get_page_intro(&title)
            .await
            .map_err(|e| format!("Error fetching page intro: {}", e).into())
    }
    ...
    pub async fn execute_tool_call(tool_call: &ToolCall) -> Result<String, Box<dyn Error>> {
        if tool_call.function.name == "calculate" {
            ...
            let api_key = dotenvy::var("SERP_API_KEY").map_err(|_| "Missing API key")?;
            info!("> Query: {}", query);
            let result = search_web(query, &api_key).await?;
            Ok(format!("<tool_result>{}</tool_result>", result))
        } else if tool_call.function.name == "search_wikipedia" {
            info!("> Tool call: search_wikipedia");
            let args: Value = serde_json::from_str(&tool_call.function.arguments)?;
            let query = args["query"].as_str().ok_or("Missing query for search_wikipedia")?;
            info!("> Query: {}", query);
            let result = search_wikipedia(query).await?;
            Ok(format!("<tool_result>{}</tool_result>", result))
        } else {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    db.add_document("The secret code to access the project is 'quantum_leap_42'.")
        .await?;
    db.add_document("Alice is the lead engineer for the new 'Orion' feature.")
        .await?;
    db.add_document("The project deadline has been moved to next Friday.")
        .await?;
    info!("\nVector DB is ready.");
    ...
    loop {
        ...
        println!("Choose mode: (1) Simple response, (2) Tool Use, (3) RAG");
        print!("Mode: ");
        io::stdout().flush()?;
